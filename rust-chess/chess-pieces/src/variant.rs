...
impl PieceVariant {
    ...
    pub fn set_position(&mut self, pos: Position) {
        match self {
            PieceVariant::Empty(p) => p.base_mut().position = pos,
            PieceVariant::Bishop(p) => p.base_mut().position = pos,
            PieceVariant::King(p) => p.base_mut().position = pos,
            PieceVariant::Knight(p) => p.base_mut().position = pos,
            PieceVariant::Pawn(p) => p.base_mut().position = pos,
            PieceVariant::Queen(p) => p.base_mut().position = pos,
            PieceVariant::Rook(p) => p.base_mut().position = pos,
        }
    }
    
    /// Get drawing representation of the piece
